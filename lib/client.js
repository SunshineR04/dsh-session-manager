...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      deleteRunningBody: '会话「{title}」正在运行，无法删除。请先切换到该会话并结束当前任务。',
      confirm: '删除',
      cancel: '取消',
    ...
    const en = {
      ...
      deleteRunningBody: 'Session “{title}” is running and cannot be deleted. Open it and finish the current task first.',
      confirm: 'Delete',
      cancel: 'Cancel',
    ...
    // Spin keyframes for the refresh buttons — inline styles cannot declare
    // @keyframes, so inject the stylesheet once (idempotent).
    const ensureSpinStyle = () => {
      try {
        ...
        style.textContent = '@keyframes sm-spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}'
    ...
    function SessionManagerSection(props) {
      ...
      const toast = (kind, text) => {
        ...
        setTimeout(() => dispatchToast({ type: 'drop', id }), 5000)
      }
      ...
      // Refresh the archived rows: re-pull the client session store and spin
      // the button icon until the (single-flight) refresh settles.
      ...
      const remove = async (sessionId, title, running) => {
        ...
        try {
          ...
          if (message.includes('session/running')) setError(format(t, 'runningRefused', { title }))
          else setError(`${t('errorPrefix')}${message}`)
        } finally {
        ...
        decisionPromise.then(async (decision) => {
          ...
          try {
            ...
            if (value?.openAtDelete === true) {
              ...
              if (!gone) fallbackToast(handlers.t('refreshFailed'))
              fallbackToast(format(handlers.t, 'deleteOk', { title: session.title || session.sessionId }))
            }
          } catch (reason) {
            try { handlers.onError(reason) } catch { /* noop */ }
            const message = errorMessage(reason)
            const text = message.includes('session/running')
              ? format(handlers.t, 'runningRefused', { title: session.title || session.sessionId })
              : `${handlers.t('errorPrefix')}${message}`
            fallbackToast(text)
          } finally {
            closeMenu()
          }
    ...
    function apply(ctx) {
      ...
      const rpc = async (endpoint, payload = {}) => {
        ...
        if (result === undefined || typeof result !== 'object' || result.ok !== true) {
          const reason = result && typeof result === 'object' && result.error !== undefined
            ? `${result.error.code}: ${result.error.message}`
            : 'session-manager RPC failed'
      ...
      try {
        ...
        if (remote !== undefined && typeof remote.$on === 'function') {
          remote.$on('api-session/removed', () => {
            const sessions = getSessions()
            if (sessions && typeof sessions.refreshList === 'function') sessions.refreshList()
          })
        }
      } catch {
