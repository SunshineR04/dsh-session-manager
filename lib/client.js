  ...
  factory: (require) => {
    ...
    function openConfirm({ title, body, confirmLabel, cancelLabel, danger = false, onRenderError = null }) {
      return new Promise((resolve) => {
        ...
        confirmOpen = true
        const titleId = `dsh-session-manager-confirm-title-${++confirmSeq}`
        ...
        // must not be allowed to walk into it.
        ...
        function finish(value) {
          ...
          // button, the menu trigger) instead of dropping it on <body> — the
          // menu path's opener is usually already unmounted, hence the checks.
          try {
            if (opener !== null && opener !== overlay && typeof opener.focus === 'function' && document.contains(opener)) opener.focus()
          } catch { /* noop */ }
          resolve(value)
        }
    ...
    function apply(ctx) {
      ...
      // the user opens the "…" menu. The value must therefore be settled BEFORE
      // that render: a single transient ping failure would otherwise hide the row
      // for the rest of the page, and no later update can reach the component.
      // Hence two short retries, and no reliance on "reopening the menu re-reads".
      let menuEnabled = null
      let menuPingTimer = null
      const settleMenuEnabled = (attempt) => {
        ping().then((value) => { menuEnabled = value !== false }).catch(() => {
          if (attempt < 2) {
