...
export function registerCommands(ctx, manager) {
  // The commands service may start after this plugin (the plugin declares no
  // host inject and activates as soon as its row loads) — cordis' inject()
  // waits for availability, closing that race. On a host that never mounts
  // the service the callback stays pending and the other surfaces are
  // unaffected.
  try {
  ...
  ${USAGE}` }
    ...
    ctx.logger?.warn?.(`session-manager: commands service unavailable: ${errorMessage(error)}`)
  }
}
...
export function registerTools(ctx, manager) {
  // Same readiness race as registerCommands: wait for the tools service
  // instead of reading it once at apply time.
  try {
